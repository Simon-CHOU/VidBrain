...
def process_pipeline(  # noqa: C901
    ...
    try:
        ...
        logger.info(
            "[Pipeline] 阶段 2/4 - 扫描知识库: %s, 发现 %d 篇笔记 (缓存)",
            video_name,
            len(existing_notes),
        )
        ...
        if feedback_context:
            logger.info(
                "[Pipeline] 用户反馈信号已提取 (%d 篇编辑, %d 篇审核)",
                feedback_signals["edited_count"],
                feedback_signals["reviewed_count"],
            )
        ...
        if cfg.semi:
            ...
            if (
                ...
                and embed_store_for_check is not None
