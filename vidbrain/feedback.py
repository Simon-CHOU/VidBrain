...
def detect_user_edits(vault_path: str) -> list[dict]:
    """扫描 Vault 中用户编辑过的笔记。
    ...
    logger.info(
        ...
        len(results), edited_count, reviewed_count,
    )
    return results
...
def extract_feedback_signals(vault_path: str, edited_notes: list[dict]) -> dict:
    ...
    from vidbrain.refiner import parse_links
    ...
    logger.info(
        ...
        len(preferred_links), len(user_edited), reviewed_count,
    )
    return {
...
def get_feedback_context(feedback_signals: dict) -> str:
    ...
    if preferred:
        links_str = ", ".join(f"[[{l}]]" for l in preferred)
    ...
    if reviewed_count > 0:
        parts.append(
            f"有 {reviewed_count} 篇笔记已被用户审核通过，"
            "请参考其风格和链接选择"
        )
