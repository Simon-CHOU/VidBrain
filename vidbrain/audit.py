...
class AuditLogger:
    ...
    def task_status_change(
        self, video_id: str, video_name: str, from_status: str, to_status: str,
        ...
        self.log("task_status_change", "pipeline", d, status=status,
    ...
    def api_call(
        self, provider: str, endpoint: str, duration_ms: float,
        success: bool, error: str = "", details: dict[str, Any] | None = None,
    ) -> None:
        """记录 API 调用。"""
        d = details or {}
        d.update({
    ...
    def classification(
        self, video_id: str, video_name: str, category: str, reason: str,
    ) -> None:
        """记录分类决策。"""
        self.log("classification", "classifier",
                 {"category": category, "reason": reason},
                 video_id=video_id, video_name=video_name)

    def queue_backpressure(self, queue_size: int, max_size: int) -> None:
        """记录队列积压事件。"""
        self.log("queue_backpressure", "watcher",
                 {"queue_size": queue_size, "max_size": max_size},
                 status="warning")

    def error(
        self, component: str, error_message: str,
        ...
        self.log("error", component, d, status="failure",
                 video_id=video_id, video_name=video_name)
